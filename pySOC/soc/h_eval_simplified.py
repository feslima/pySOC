...
def h_exact_local_method(Gy, Gyd, Juu, Jud, d, Wd, Wny):
    ...
    H_exact = mrdivide(Gy.conj().T, (Ft @ Ft.conj().T)) #Simplified H evaluation, based on Yelchuru and Skogestad (2012)
...
def h_exact_local_method_ss(Gy, Gyd, Juu, Jud, d, Wd, me, ss_size, nc_user):
    ...
    for u in np.arange(sset_bnb.shape[0]):
        ...
        H_exact_ss = mrdivide(Gy_ss.conj().T,
                            (Ft_ss @ Ft_ss.conj().T))  # Simplified H evaluation, based on Yelchuru and Skogestad (2012)


        H_exact_ss = (H_exact_ss / np.linalg.norm(H_exact_ss, ord='fro'))
